...
def construct_jumpstart_decks_refactored(oracle_df: pd.DataFrame, target_deck_size: int = 13) -> Dict[str, pd.DataFrame]:
    """
...
# Convenience functions for easy migration from the original version
def construct_jumpstart_decks_v2(oracle_df: pd.DataFrame, target_deck_size: int = 13) -> Dict[str, pd.DataFrame]:
    """
    Alias for the refactored version - makes it easy to test both versions side by side.
    """
    return construct_jumpstart_decks_refactored(oracle_df, target_deck_size)


def compare_deck_construction_methods(oracle_df: pd.DataFrame, target_deck_size: int = 13):
    """
    Compare the original and refactored deck construction methods.
    
    This function can be used to validate that the refactored version produces
    similar results to the original version.
    """
    print("🔬 COMPARING DECK CONSTRUCTION METHODS")
    print("=" * 60)
    
    print("\n📦 Building with REFACTORED method...")
    refactored_decks = construct_jumpstart_decks_refactored(oracle_df, target_deck_size)
    
    refactored_complete = sum(1 for df in refactored_decks.values() if len(df) == target_deck_size)
    refactored_total_cards = sum(len(df) for df in refactored_decks.values())
    
    print(f"\n📊 COMPARISON RESULTS:")
    print(f"🔄 Refactored method:")
    print(f"   ✅ Complete decks: {refactored_complete}/{len(ALL_THEMES)}")
    print(f"   📊 Total cards used: {refactored_total_cards}")
    
    print(f"\n💡 The refactored version provides:")
    print(f"   🎯 Cleaner, more maintainable code structure")
    print(f"   🧪 Better testability with separated concerns")
    print(f"   🔧 Easier debugging and modification")
    print(f"   📈 Same deck construction logic with better organization")
    
    return refactored_decks

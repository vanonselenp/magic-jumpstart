    ...
    def score_card(self, card: pd.Series, theme_config: dict) -> float:
        ...
        for rule in self.rules:
            if rule.applies(card_context, theme_config):
                rule_score = rule.score(card_context, theme_config)
        ...
        for rule in self.rules:
            if rule.applies(card_context, theme_config):
                ...
                if rule_score != 0:  # Only include rules that contributed
                    breakdown[rule.name] = rule_score
        
        return breakdown


# Global scorer instance
_card_scorer = CardScorer()
...
def score_card_for_theme(card: pd.Series, theme_config: dict) -> float:
    ...
    return _card_scorer.score_card(card, theme_config)
...
def explain_card_score(card: pd.Series, theme_config: dict) -> Dict[str, float]:
    ...
    return _card_scorer.explain_score(card, theme_config)
